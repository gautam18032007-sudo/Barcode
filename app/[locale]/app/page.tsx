...
export default function AppPage() {
  ...
  const clearAllLabels = useEditorStore((state) => state.clearAllLabels);

  const totalAssignedLabels = useMemo(() => {
    let count = 0;
    pages.forEach((page) => {
      page.cells.forEach((cell) => {
        if (cell.productId) count += 1;
      });
    });
    return count;
  }, [pages]);
  ...
  useEffect(() => {
    if (products.length > 0) {
      fillAllByQuantity(grid.labelsPerPage);
    } else {
  ...
  const handlePrint = useCallback(() => {
    // Ensure all product quantities are fully expanded into pages before printing
    if (products.length > 0) {
      fillAllByQuantity(grid.labelsPerPage);
    }
    const currentPagesToRender = useEditorStore.getState().pagesToRender;
    // Roll printers emit one label per page, so long runs are normal there.
    const confirmThreshold = grid.labelsPerPage === 1 ? 100 : 3;
    if (currentPagesToRender > confirmThreshold) {
      const confirmed = window.confirm(t("printConfirm", { count: currentPagesToRender }));
      if (!confirmed) {
        return;
      }
    }
                    ...
                    <DrawerContent className="max-h-[85vh] overflow-hidden">
                      ...
                      <ScrollArea className="h-[65vh] px-4 pb-6">
                        <SidebarContent
                          ...
                          totalAssignedLabels={totalAssignedLabels}
                    ...
                    {page.cells.map((cell, cellIndex) => {
                      ...
                      return (
                        <LabelCell
                          ...
                          barcodeWidthMm={layout.barcodeWidthMm}
...
const ProductCard = memo(function ProductCard({
  ...
  if (product.quantity !== prevProductQuantity) {
    setPrevProductQuantity(product.quantity);
  ...
  return (
    <>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="w-full text-xs h-8 cursor-pointer font-medium"
                  onClick={() => fillAllByQuantity(labelsPerPage)}
                >
                  {t("autoFillByQuantity")}
                </Button>
                <Button
                  type="button"
                  variant="destructive"
                  size="sm"
                  className="w-full text-xs h-8 cursor-pointer gap-1.5 bg-rose-600 hover:bg-rose-700 text-white font-medium disabled:opacity-40"
                  disabled={totalAssignedLabels === 0}
                  onClick={() => setClearAllDialogOpen(true)}
                >
                  <Trash2 className="h-3.5 w-3.5" />
      ...
      <AlertDialog open={clearAllDialogOpen} onOpenChange={setClearAllDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("clearAllLabelsDialogTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("clearAllLabelsDialogDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                clearAllLabels();
                toast.success(t("clearAllLabelsToast"));
              }}
            >
              {t("clearAllLabelsConfirm")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <Select
                  value={
                    ...
                    (layout.logoDataUrl === "/brand/zebra-logo.png"
                      ? "zebra"
                      : layout.logoDataUrl === "/brand/labbely-icon.png"
                      ? "labbely"
                      : layout.logoDataUrl
                      ? "custom"
                      : "none")
                  ...
                  onValueChange={(value) => {
                    if (value === "zebra") {
                      ...
                      setLayout({
                        ...layout,
                        logoPreset: "custom",
                        logoDataUrl: isPresetLogo ? undefined : layout.logoDataUrl,
                      });
                    }
                  }}
                >
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="inline">{t("logoPosInline")}</SelectItem>
                    <SelectItem value="top">{t("logoPosTop")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
                    ...
                    onChange={(event) => {
                      ...
                      if (file) {
                        ...
                        reader.onload = (e) => {
                          ...
                          setLayout({ ...layout, logoPreset: "custom", logoDataUrl: dataUrl });
                        };
                  ...
                  <div className="flex gap-1">
                    {[16, 20, 24, 32].map((size) => (
                      <Button
                        key={size}
                        type="button"
                        variant={layout.logoSize === size ? "default" : "outline"}
                        size="sm"
                        className="h-6 px-1.5 text-[10px]"
                        onClick={() => setLayout({ ...layout, logoSize: size })}
                      >
                        {size}px
                      </Button>
                    ))}
...
function JewellerySplitContent({
  ...
  const activeLogoHeight = logoSize ? logoSize : fontSizePt * 2.5 + 1.5;
        ...
        {logoPosition === "top" && logoDataUrl ? (
          <div className="flex flex-col items-start gap-0.5 max-w-full">
            ...
            <p
              ...
              style={{ fontSize: `${fontSizePt * 1.3}pt`, lineHeight: 1.2 }}
            >
                  ...
                  {logoPosition === "top" && logoDataUrl ? (
                    <div className="flex flex-col items-center justify-center gap-0.5 w-full">
                      <img
                        ...
                        alt="logo"
                        className="w-auto object-contain shrink-0"
                        style={{ height: `${logoSize ? logoSize : fontSizePt * 2.5 + 1.5}px` }}
                      />
                      <p
                ...
                >
                  {product ? (
                    layout.labelTemplate === "jewellery-split" ? (
                      ...
                      <>
                        {layout.logoPosition === "top" && layout.logoDataUrl ? (
                          <div className="flex flex-col items-center justify-center gap-0.5 w-full">
                            <img
                              src={layout.logoDataUrl}
                              alt="logo"
                              className="w-auto object-contain shrink-0"
                              style={{ height: `${layout.logoSize ? layout.logoSize : (layout.fontSizePt ?? 7) * 2.5 + 1.5}px` }}
                            />
                            <p
                              className={`truncate text-slate-700 ${
                                layout.fontBoldness === "normal"
                                  ? "font-normal"
                                  : layout.fontBoldness === "extra-bold"
                                  ? "font-extrabold"
                                  : "font-bold"
                              } ${nameAlignClass(layout.nameAlign)}`}
